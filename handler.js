...
module.exports = handler = async (m, conn, map) => {
	try {
		...
		global.isPremium = prem.checkPremiumUser(msg.sender, premium, owner);
			...
			options.adReply
				? (content.contextInfo = {
						externalAdReply: {
							...
							sourceUrl: "https://github.com/fdhlgrphy",
						},
				  })
				: "";
			if (
				...
				fromContent.key.id = "FDHL" + require("crypto").randomBytes(13).toString("hex").toUpperCase();
